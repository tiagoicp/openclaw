import { i as normalizeLowercaseStringOrEmpty$2, o as normalizeOptionalLowercaseString, s as normalizeOptionalString } from "./string-coerce-BUSzWgUA.js";
...
import { c as sendGoogleChatMessage, d as verifyGoogleChatRequest, i as downloadGoogleChatMedia, l as updateGoogleChatMessage, n as deleteGoogleChatMessage, s as probeGoogleChat, u as uploadGoogleChatAttachment } from "./api-CDH5P7rR.js";
...
function normalizeUserId$1(raw) {
	...
	if (!trimmed) return "";
	return normalizeLowercaseStringOrEmpty$1(trimmed.replace(/^users\//i, ""));
}
...
function isSenderAllowed(senderId, senderEmail, allowFrom, allowNameMatching = false) {
	...
	const normalizedEmail = normalizeLowercaseStringOrEmpty$1(senderEmail ?? "");
	return allowFrom.some((entry) => {
		const normalized = normalizeLowercaseStringOrEmpty$1(entry);
...
function normalizeUserId(raw) {
	...
	if (!trimmed) return "";
	return normalizeLowercaseStringOrEmpty$2(trimmed.replace(/^users\//i, ""));
}
function resolveGroupConfig(params) {
	...
	const normalizedGroupName = normalizeLowercaseStringOrEmpty$2(groupName ?? "");
	const deprecatedNameMatch = !entry && Boolean(groupName && keys.some((key) => {
		const trimmed = key.trim();
		if (!trimmed || trimmed === "*" || /^spaces\//i.test(trimmed)) return false;
		return trimmed === groupName || normalizeLowercaseStringOrEmpty$2(trimmed) === normalizedGroupName;
	}));
...
function warnDeprecatedUsersEmailEntries(logVerbose, entries) {
	...
	const key = deprecated.map((v) => normalizeLowercaseStringOrEmpty$2(v)).toSorted((a, b) => a.localeCompare(b)).join(",");
...
function warnMutableGroupKeysConfigured(logVerbose, groups) {
	...
	const warningKey = mutableKeys.map((key) => normalizeLowercaseStringOrEmpty$2(key)).toSorted((a, b) => a.localeCompare(b)).join(",");

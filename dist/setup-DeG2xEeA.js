import { n as DEFAULT_GATEWAY_PORT, u as resolveGatewayPort } from "./paths-D_QmduAc.js";
import { m as defaultRuntime } from "./subsystem-Dm-AQqmI.js";
import { y as resolveUserPath } from "./utils-CIAfMgvq.js";
import { $s as formatPluginCompatibilityNotice, Ys as buildPluginCompatibilityNotices } from "./auth-profiles-CPtwVkYW.js";
import { d as readConfigFileSnapshot, g as writeConfigFile } from "./io-BLrYinYw.js";
import { l as normalizeSecretInputString } from "./types.secrets-Mh0km-xK.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-Bgfs5Gx5.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BLMMb8UB.js");
		const { logConfigUpdated } = await import("./logging-xDsDEt_l.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-BoIhd6Fh.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-D_wTkK1E.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-t9AKSg0S.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-C5gqJwmC.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-BKhyU4W_.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CBOLAqjs.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-WcXsD6rz.js");
		const { setupChannels } = await import("./onboard-channels-CfdPoigg.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-YH83kxLc.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BYR0Sw70.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-CyzwcUpN.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-dt9Wm1be.js");

...
import { $s as formatPluginCompatibilityNotice, Ys as buildPluginCompatibilityNotices } from "./auth-profiles-BqVjFbSG.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-C2ReD4ux.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-N9isAD0k.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-CTFukA8v.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-Q0K8ZDud.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-C4P9WL41.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DUvRdQ1i.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CDy88VEh.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-obR2KdpG.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-DeImiLXa.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BeQQci9_.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-PglIVs7d.js");

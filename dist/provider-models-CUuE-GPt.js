...
import { C as sleep } from "./utils-DHW4u72m.js";
import { o as ensureModelAllowlistEntry, r as applyProviderConfigWithDefaultModel, t as applyAgentDefaultModelPrimary } from "./provider-onboard-CTDjrzVM.js";
...
function toModelDefinition$1(entry) {
	return {
		id: entry.id,
		name: entry.name,
...
function buildXaiModelDefinition() {
	return toModelDefinition$1(XAI_MODEL_CATALOG.find((entry) => entry.id === "grok-4") ?? {
		id: "grok-4",
		name: "Grok 4",
		reasoning: false,
...
function buildXaiCatalogModels() {
	return XAI_MODEL_CATALOG.map((entry) => toModelDefinition$1(entry));
}
function resolveXaiCatalogEntry(modelId) {
	const lower = modelId.trim().toLowerCase();
	const exact = XAI_MODEL_CATALOG.find((entry) => entry.id.toLowerCase() === lower);
	if (exact) return toModelDefinition$1(exact);
	if (lower.includes("multi-agent")) return;
	if (lower.startsWith("grok-code-fast")) return toModelDefinition$1({
		id: modelId.trim(),
		name: modelId.trim(),
		reasoning: true,
		contextWindow: XAI_CODE_CONTEXT_WINDOW
	});
	if (lower.startsWith("grok-4.20") || lower.startsWith("grok-4-1") || lower.startsWith("grok-4-fast")) return toModelDefinition$1({
		id: modelId.trim(),
		name: modelId.trim(),
		reasoning: !lower.includes("non-reasoning"),
		contextWindow: XAI_LARGE_CONTEXT_WINDOW
	});
	if (lower.startsWith("grok-4")) return toModelDefinition$1({
		id: modelId.trim(),
		name: modelId.trim(),
...
//#endregion
export { isReasoningModelHeuristic as $, applyXaiModelCompat as $t, TOGETHER_MODEL_CATALOG as A, MINIMAX_HOSTED_MODEL_ID as At, buildChutesModelDefinition as B, applyOpenAIConfig as Bt, VENICE_DEFAULT_MODEL_REF as C, MISTRAL_DEFAULT_MODEL_ID as Ct, resolveRetryConfig as D, MINIMAX_API_BASE_URL as Dt, discoverVeniceModels as E, DEFAULT_MINIMAX_BASE_URL as Et, buildSyntheticModelDefinition as F, GOOGLE_GEMINI_DEFAULT_MODEL as Ft, buildHuggingfaceModelDefinition as G, KILOCODE_DEFAULT_COST as Gt, discoverKilocodeModels as H, cloneFirstTemplateModel as Ht, CHUTES_BASE_URL as I, OPENAI_DEFAULT_MODEL as It, OLLAMA_DEFAULT_COST as J, KILOCODE_DEFAULT_MODEL_NAME as Jt, discoverHuggingfaceModels as K, KILOCODE_DEFAULT_MAX_TOKENS as Kt, CHUTES_DEFAULT_MODEL_ID as L, OPENCODE_GO_DEFAULT_MODEL_REF as Lt, SYNTHETIC_BASE_URL as M, MINIMAX_LM_STUDIO_COST as Mt, SYNTHETIC_DEFAULT_MODEL_REF as N, buildMinimaxApiModelDefinition as Nt, retryAsync as O, MINIMAX_CN_API_BASE_URL as Ot, SYNTHETIC_MODEL_CATALOG as P, OPENCODE_ZEN_DEFAULT_MODEL_REF as Pt, fetchOllamaModels as Q, XAI_TOOL_SCHEMA_PROFILE as Qt, CHUTES_DEFAULT_MODEL_REF as R, OPENCODE_ZEN_DEFAULT_MODEL$1 as Rt, VENICE_BASE_URL as S, MISTRAL_BASE_URL as St, buildVeniceModelDefinition as T, buildMistralModelDefinition as Tt, HUGGINGFACE_BASE_URL as U, KILOCODE_BASE_URL as Ut, discoverChutesModels as V, applyPrimaryModel as Vt, HUGGINGFACE_MODEL_CATALOG as W, KILOCODE_DEFAULT_CONTEXT_WINDOW as Wt, buildOllamaModelDefinition as X, KILOCODE_MODEL_CATALOG as Xt, OLLAMA_DEFAULT_MAX_TOKENS as Y, KILOCODE_DEFAULT_MODEL_REF as Yt, enrichOllamaModelsWithContext as Z, HTML_ENTITY_TOOL_CALL_ARGUMENTS_ENCODING as Zt, BYTEPLUS_BASE_URL as _, MODELSTUDIO_DEFAULT_MODEL_ID as _t, SGLANG_DEFAULT_BASE_URL as a, XAI_DEFAULT_MODEL_ID as at, BYTEPLUS_MODEL_CATALOG as b, buildModelStudioDefaultModelDefinition as bt, VLLM_DEFAULT_API_KEY_ENV_VAR as c, buildXaiModelDefinition as ct, VLLM_PROVIDER_LABEL as d, MOONSHOT_DEFAULT_MODEL_REF as dt, hasNativeWebSearchTool as en, resolveOllamaApiBase as et, DOUBAO_BASE_URL as f, applyMoonshotConfig as ft, buildDoubaoModelDefinition as g, MODELSTUDIO_CN_BASE_URL as gt, DOUBAO_MODEL_CATALOG as h, buildMoonshotProvider as ht, SGLANG_DEFAULT_API_KEY_ENV_VAR as i, OLLAMA_DEFAULT_BASE_URL as in, XAI_BASE_URL as it, buildTogetherModelDefinition as j, MINIMAX_HOSTED_MODEL_REF as jt, TOGETHER_BASE_URL as k, MINIMAX_HOSTED_COST as kt, VLLM_DEFAULT_BASE_URL as l, resolveXaiCatalogEntry as lt, DOUBAO_CODING_MODEL_CATALOG as m, MOONSHOT_BASE_URL as mt, VERCEL_AI_GATEWAY_BASE_URL as n, resolveToolCallArgumentsEncoding as nn, buildCloudflareAiGatewayModelDefinition as nt, SGLANG_MODEL_PLACEHOLDER as o, XAI_DEFAULT_MODEL_REF as ot, DOUBAO_CODING_BASE_URL as p, applyMoonshotConfigCn as pt, OLLAMA_DEFAULT_CONTEXT_WINDOW as q, KILOCODE_DEFAULT_MODEL_ID as qt, discoverVercelAiGatewayModels as r, usesXaiToolSchemaProfile as rn, resolveCloudflareAiGatewayBaseUrl as rt, SGLANG_PROVIDER_LABEL as s, buildXaiCatalogModels as st, buildKilocodeModelDefinition as t, normalizeModelCompat as tn, CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_REF as tt, VLLM_MODEL_PLACEHOLDER as u, MOONSHOT_CN_BASE_URL as ut, BYTEPLUS_CODING_BASE_URL as v, MODELSTUDIO_DEFAULT_MODEL_REF as vt, VENICE_MODEL_CATALOG as w, MISTRAL_DEFAULT_MODEL_REF as wt, buildBytePlusModelDefinition as x, buildModelStudioModelDefinition as xt, BYTEPLUS_CODING_MODEL_CATALOG as y, MODELSTUDIO_GLOBAL_BASE_URL as yt, CHUTES_MODEL_CATALOG as z, applyGoogleGeminiModelDefault as zt };

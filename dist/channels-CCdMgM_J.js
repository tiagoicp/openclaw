...
import { n as buildReadOnlySourceChannelAccountSnapshot, t as buildChannelAccountSnapshot } from "./status-5DqA77FN.js";
...
import { i as describeBinding, n as applyAgentBindings, t as requireValidConfigSnapshot } from "./config-validation-ByNEUI3f.js";
import { t as isCatalogChannelInstalled } from "./discovery-DBZJMnND.js";
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (shouldUseWizard(params)) {
		const [{ buildAgentSummaries }, { setupChannels }] = await Promise.all([import("./agents.config-_3WPHR00.js"), import("./onboard-channels-obR2KdpG.js")]);
	...
	const loadScopedPlugin = async (channelId, pluginId) => {
		...
		const { loadChannelSetupPluginRegistrySnapshotForChannel } = await import("./plugin-install-C5RNSNd7.js");
	...
	if (!channel && catalogEntry) {
		...
		if (!isCatalogChannelInstalled({
			...
			const { ensureChannelSetupPluginInstalled } = await import("./plugin-install-C5RNSNd7.js");
			const prompter = createClackPrompter();

...
import { g as writeConfigFile, l as readConfigFileSnapshot } from "./io-CW6SWMPF.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-B3_NRgEI.js");
...
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-DwFmZIUZ.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-D71i0DcO.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-CEupgiJ8.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-H1aFlAXQ.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-B_7A5Aao.js");
	...
	if (authChoiceFromPrompt) {
		...
		const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-VDbMEzxB.js");
	...
	if (authChoice === "custom-api-key") {
		const { promptCustomApiConfig } = await import("./onboard-custom-DrgzjSoT.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-W4MiW8Ol.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-D-m8SWq5.js");
		const { setupChannels } = await import("./onboard-channels-Bn13PPwA.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BFS866CR.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BTm0gnjd.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-vJIH2DkC.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-DJxcVLW0.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DfxMRQB-.js");

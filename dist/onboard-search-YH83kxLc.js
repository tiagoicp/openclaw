...
import { Uf as resolvePluginWebSearchProviders } from "./auth-profiles-CPtwVkYW.js";
...
import { a as hasConfiguredSecretInput, l as normalizeSecretInputString, t as DEFAULT_SECRET_PROVIDER_ALIAS } from "./types.secrets-Mh0km-xK.js";
...
const SEARCH_PROVIDER_OPTIONS = resolvePluginWebSearchProviders({ bundledAllowlistCompat: true }).map((provider) => ({
	value: provider.id,
...
function rawKeyValue(config, provider) {
	...
	return entry?.getConfiguredCredentialValue?.(config) ?? entry?.getCredentialValue(config.tools?.web?.search);
}
...
function applySearchKey(config, provider, key) {
	...
	if (providerEntry?.setConfiguredCredentialValue) providerEntry.setConfiguredCredentialValue(nextBase, key);
	else {
		const search = nextBase.tools?.web?.search;
		if (providerEntry && search) providerEntry.setCredentialValue(search, key);
	}

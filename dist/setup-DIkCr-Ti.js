...
import { $s as formatPluginCompatibilityNotice, Ys as buildPluginCompatibilityNotices } from "./auth-profiles-C1V2x6_A.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-DpRa4egE.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BerkVTen.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-D9jGo_di.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-E39jUs9a.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-2J34NwlK.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-qRcvc8Lp.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DDgjTHj5.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-CWHie3YB.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-T1EwaJh_.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-DsI-V9_a.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DNFogdaq.js");

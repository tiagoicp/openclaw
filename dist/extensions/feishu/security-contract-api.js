//#region extensions/feishu/security-contract-api.ts
...
function hasConfiguredSecretInput(value) {
	...
	return Boolean(record) && hasNonEmptyString(record?.source) && hasNonEmptyString(record?.provider) && hasNonEmptyString(record?.id);
}
function isFeishuDocToolEnabled(cfg) {
	...
	for (const accountValue of Object.values(accounts)) {
		...
		if ((hasNonEmptyString(account.appId) || baseAppId) && (hasConfiguredSecretInput(account.appSecret) || baseAppSecret)) return true;
	}
	return false;
}

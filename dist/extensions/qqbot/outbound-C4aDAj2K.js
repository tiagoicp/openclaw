...
import { fetchRemoteMedia as fetchRemoteMedia$1 } from "openclaw/plugin-sdk/media-runtime";
...
async function sendC2CImageMessage(appId, accessToken, openid, imageUrl, msgId, content, localPath) {
	...
	if (isBase64) {
		...
		uploadResult = await uploadC2CMedia(accessToken, openid, 1, void 0, matches[2], false);
	} else uploadResult = await uploadC2CMedia(accessToken, openid, 1, imageUrl, void 0, false);
	const meta = {
		text: content,
		mediaType: "image",
...
async function sendGroupImageMessage(appId, accessToken, groupOpenid, imageUrl, msgId, content) {
	...
	if (imageUrl.startsWith("data:")) {
		const matches = imageUrl.match(/^data:([^;]+);base64,(.+)$/);
		if (!matches) throw new Error("Invalid Base64 Data URL format");
		uploadResult = await uploadGroupMedia(accessToken, groupOpenid, 1, void 0, matches[2], false);
	} else uploadResult = await uploadGroupMedia(accessToken, groupOpenid, 1, imageUrl, void 0, false);
	return sendGroupMediaMessage(accessToken, groupOpenid, uploadResult.file_info, msgId, content);
}
async function sendC2CVoiceMessage(appId, accessToken, openid, voiceBase64, voiceUrl, msgId, ttsText, filePath) {
	return sendC2CMediaMessage(appId, accessToken, openid, (await uploadC2CMedia(accessToken, openid, 3, voiceUrl, voiceBase64, false)).file_info, msgId, void 0, {
		mediaType: "voice",
		...ttsText ? { ttsText } : {},
...
async function sendGroupVoiceMessage(appId, accessToken, groupOpenid, voiceBase64, voiceUrl, msgId) {
	return sendGroupMediaMessage(accessToken, groupOpenid, (await uploadGroupMedia(accessToken, groupOpenid, 3, voiceUrl, voiceBase64, false)).file_info, msgId);
}
async function sendC2CFileMessage(appId, accessToken, openid, fileBase64, fileUrl, msgId, fileName, localFilePath) {
	return sendC2CMediaMessage(appId, accessToken, openid, (await uploadC2CMedia(accessToken, openid, 4, fileUrl, fileBase64, false, fileName)).file_info, msgId, void 0, {
		mediaType: "file",
		mediaUrl: fileUrl,
...
async function sendGroupFileMessage(appId, accessToken, groupOpenid, fileBase64, fileUrl, msgId, fileName) {
	return sendGroupMediaMessage(accessToken, groupOpenid, (await uploadGroupMedia(accessToken, groupOpenid, 4, fileUrl, fileBase64, false, fileName)).file_info, msgId);
}
async function sendC2CVideoMessage(appId, accessToken, openid, videoUrl, videoBase64, msgId, content, localPath) {
	return sendC2CMediaMessage(appId, accessToken, openid, (await uploadC2CMedia(accessToken, openid, 2, videoUrl, videoBase64, false)).file_info, msgId, content, {
		text: content,
		mediaType: "video",
...
async function sendGroupVideoMessage(appId, accessToken, groupOpenid, videoUrl, videoBase64, msgId, content) {
	return sendGroupMediaMessage(accessToken, groupOpenid, (await uploadGroupMedia(accessToken, groupOpenid, 2, videoUrl, videoBase64, false)).file_info, msgId, content);
}
...
let mpegDecoderConstructorPromise = null;
async function loadMpegDecoderConstructor() {
...
async function downloadFile(url, destDir, originalFilename) {
	try {
		...
		const fetched = await fetchRemoteMedia$1({
			url: parsedUrl.toString(),
			filePathHint: originalFilename,
			ssrfPolicy: QQBOT_MEDIA_SSRF_POLICY
		});
		let filename = normalizeOptionalString$1(originalFilename) ?? "";
		if (!filename) filename = (normalizeOptionalString$1(fetched.fileName) ?? path$2.basename(parsedUrl.pathname)) || "download";
		const ts = Date.now();

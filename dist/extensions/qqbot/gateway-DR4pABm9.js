...
import { a as recordKnownUser, c as matchSlashCommand, d as formatQQBotAllowFrom, l as TEXT_CHUNK_LIMIT, n as flushKnownUsers, u as chunkText } from "./known-users-BWqNiDhg.js";
...
function estimateBase64DecodedBytes(base64) {
	let effectiveLen = 0;
	for (let i = 0; i < base64.length; i += 1) if (base64.charCodeAt(i) > 32) effectiveLen += 1;
...
function buildAttachmentSummaries(attachments, localPaths) {
	...
	return attachments.map((att, idx) => {
		const ct = normalizeLowercaseStringOrEmpty$2(att.content_type);
		let type = "unknown";
...
async function parseAndSendMediaTags(replyText, event, actx, sendWithRetry, consumeQuoteRef) {
	...
	while ((match = regex2.exec(text)) !== null) {
		...
		let mediaPath = decodeMediaPath(normalizeOptionalString$1(match[2]) ?? "", log, prefix);
		if (mediaPath) {
			const itemType = {
				qqmedia: "media",
				qqvoice: "voice",

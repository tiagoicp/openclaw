...
import { _ as throwWebSearchApiError, c as normalizeFreshness, d as readConfiguredSecretString, f as readProviderEnvValue, g as resolveSiteName, h as resolveSearchTimeoutSeconds, l as normalizeToIsoDate, m as resolveSearchCount, o as buildSearchCacheKey, p as resolveSearchCacheTtlMs, s as isoToPerplexityDate, u as readCachedSearchPayload, v as withTrustedWebSearchEndpoint, y as writeCachedSearchPayload } from "../../plugins-allowlist-CQiNlTMp.js";
...
import { a as resolveProviderWebSearchPluginConfig, o as setProviderWebSearchPluginConfigValue } from "../../provider-web-search-D6W3obMm.js";
...
function resolvePerplexityConfig(config, searchConfig) {
...
function resolveRuntimeTransport(params) {
	const scoped = resolvePerplexityConfig(params.config, params.searchConfig);
...
function createPerplexityToolDefinition(config, searchConfig, runtimeTransport) {
	...
	return {
		...
		execute: async (args) => {
			...
			if (!runtime.apiKey) return {
				...
				message: "web_search (perplexity) needs an API key. Set PERPLEXITY_API_KEY or OPENROUTER_API_KEY in the Gateway environment, or configure plugins.entries.perplexity.config.webSearch.apiKey.",
...
function createPerplexityWebSearchProvider() {
	return {
		...
		getCredentialValue: (searchConfig) => {
			const perplexity = searchConfig?.perplexity;
			return perplexity && typeof perplexity === "object" && !Array.isArray(perplexity) ? perplexity.apiKey : void 0;
		},
		...
		createTool: (ctx) => createPerplexityToolDefinition(ctx.config, ctx.searchConfig, ctx.runtimeMetadata?.perplexityTransport)
	};
}
//#endregion

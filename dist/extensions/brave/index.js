...
import { c as normalizeFreshness, d as readConfiguredSecretString, f as readProviderEnvValue, g as resolveSiteName, h as resolveSearchTimeoutSeconds, l as normalizeToIsoDate, m as resolveSearchCount, o as buildSearchCacheKey, p as resolveSearchCacheTtlMs, u as readCachedSearchPayload, v as withTrustedWebSearchEndpoint, y as writeCachedSearchPayload } from "../../plugins-allowlist-CQiNlTMp.js";
...
function resolveBraveApiKey(config, searchConfig) {
	return readConfiguredSecretString(resolveBraveConfig(config, searchConfig).apiKey, "plugins.entries.brave.config.webSearch.apiKey") ?? readConfiguredSecretString(searchConfig?.apiKey, "tools.web.search.apiKey") ?? readProviderEnvValue(["BRAVE_API_KEY"]);
}
...
function createBraveToolDefinition(config, searchConfig) {
	const braveMode = resolveBraveMode(resolveBraveConfig(config, searchConfig));
	return {
...
function createBraveWebSearchProvider() {
	return {
		...
		createTool: (ctx) => createBraveToolDefinition(ctx.config, ctx.searchConfig)
	};
}
//#endregion

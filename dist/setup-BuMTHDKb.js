...
import { $s as formatPluginCompatibilityNotice, Ys as buildPluginCompatibilityNotices } from "./auth-profiles-Djd2VmMW.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-Cdp3lbQ-.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BK8murI7.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-7IAEghrM.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-DC_8EIZT.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-B8HAvEws.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DIDbzBkv.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CfqZGWAP.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-DfDyLIWh.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-D6Y7GsSz.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-Dzzh4giC.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-CaBGmoFZ.js");

import { n as DEFAULT_GATEWAY_PORT, u as resolveGatewayPort } from "./paths-C--RM-nt.js";
import { m as defaultRuntime } from "./subsystem-DZirmh0Z.js";
import { y as resolveUserPath } from "./utils-DHW4u72m.js";
import { $s as formatPluginCompatibilityNotice, Ys as buildPluginCompatibilityNotices } from "./auth-profiles-BwxmeQoE.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-zb9IY9Rf.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CZ-i7Sok.js");
		const { logConfigUpdated } = await import("./logging-DaIoUIgi.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-j-i93Gpd.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BHT58Y7K.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-DeQFCSJ8.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-CegQBice.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CuFX5OOz.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-C9HySE9I.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-C25E0Yq5.js");
		const { setupChannels } = await import("./onboard-channels-DtikcXQU.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-CrdzAIj3.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-D2rClPUc.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-1Nm847yg.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DgFSc_P4.js");

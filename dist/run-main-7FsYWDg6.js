...
function loadModelsCommands() {
	modelsCommandsPromise ??= import("./models-BOr3qATG.js");
...
const routedCommandDefinitions = {
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-C0jx4EDd.js");
			...
			const { statusCommand } = await import("./status-uoWdBIUf.js");
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BxW5_lyj.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-Dn6O0uFZ.js");
